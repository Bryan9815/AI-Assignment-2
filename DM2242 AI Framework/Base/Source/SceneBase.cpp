...
void SceneBase::Init()
{
	...
	meshList[GEO_MISSILE] = MeshBuilder::GenerateSphere("missile", Color(1.000f, 0.271f, 0.000f), 8, 8, 1.f);
    ...
    meshList[GEO_WARRIOR] = MeshBuilder::GenerateSphere("ball", Color(0, 0, 1), 16, 16, 1.f);
    meshList[GEO_RANGER] = MeshBuilder::GenerateSphere("ball", Color(1.0f, 0.271f, 0.000f), 16, 16, 1.f);
...
void SceneBase::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y)
{
	...
	{
		Mtx44 characterSpacing;
		characterSpacing.SetToTranslation(i * 0.65f + 0.5f, 0.5f, 0); //1.0f is the spacing of each character, you may change this value
		Mtx44 MVP = projectionStack.Top() * viewStack.Top() * modelStack.Top() * characterSpacing;
		glUniformMatrix4fv(m_parameters[U_MVP], 1, GL_FALSE, &MVP.a[0]);
...
void SceneBase::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y, Mtx44 orthoView)
{
    if (!mesh || mesh->textureID <= 0)
        return;

    glDisable(GL_DEPTH_TEST);
    Mtx44 ortho = orthoView;
    //ortho.SetToOrtho(0, 80, 0, 60, -10, 10);
    projectionStack.PushMatrix();
    projectionStack.LoadMatrix(ortho);
    viewStack.PushMatrix();
